var SocialFavorite = SuperWidget.extend({
	...
	favorite: function(el, ev) {
		...
		this.rest(SocialAPI.SocialFavoriteService.ADD, [socialAlias], function(data) {
			...
			if (error.status == "403") {
				FLUIGC.toast({
					message: "This feature cannot be marked as a favorite because it does not belong to the same cloud of networking.",
					type: 'warning'
				});
			} else {
				FLUIGC.toast({
	...
	removeSelected: function() {
		...
		this.rest(SocialAPI.SocialFavoriteService.REMOVELIST, [alias], function(data) {
			FLUIGC.toast({
				message: "Item(s) removed from favorites successfully.",
				type: 'success'
			});
			self.loadGrid();
