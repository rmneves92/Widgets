var SocialFavorite = SuperWidget.extend({
	...
	favorite: function(el, ev) {
		...
		this.rest(SocialAPI.SocialFavoriteService.ADD, [socialAlias], function(data) {
			...
			if (error.status == "403") {
				FLUIGC.toast({
					message: "Este recurso no se puede marcar como favorito porque no pertenece a la misma nube de relación.",
					type: 'warning'
				});
			} else {
				FLUIGC.toast({
	...
	unfavorite: function(el, ev) {
		...
		this.rest(SocialAPI.SocialFavoriteService.REMOVE, [socialAlias], function(data) {
			...
			$(el).removeAttr('disabled');
			FLUIGC.toast({
				message: "Error al quitar de los favoritos.",
				type: 'danger'
			});
		});
	},
	...
	buildGrid: function() {
		...
		cfg.obj = {
			...
			colNames: ["", "", "Nombre", "Tipo"], // nomes das
			// colunas do
			// grid
	...
	removeSelected: function() {
		...
		this.rest(SocialAPI.SocialFavoriteService.REMOVELIST, [alias], function(data) {
			FLUIGC.toast({
				message: "Ítem(s) eliminado(s) de los favoritos con éxito.",
				type: 'success'
			});
			self.loadGrid();
