var SocialFavorite = SuperWidget.extend({
	...
	actions: [{
		label: "${i18n.getTranslation('remove')}"
	}],
	...
	translate: {
		USER: "${i18n.getTranslation('user')}",
		COMMUNITY: "${i18n.getTranslation('community')}"
	},
	...
	favorite: function(el, ev) {
		...
		this.rest(SocialAPI.SocialFavoriteService.ADD, [socialAlias], function(data) {
			...
			if (error.status == "403") {
				FLUIGC.toast({
					message: "${i18n.getTranslation('message.user.in.another.cloud')}",
					type: 'warning'
				});
			} else {
				FLUIGC.toast({
					message: "${i18n.getTranslation('error.adding')}",
					type: 'danger'
				});
			}
		});
	...
	unfavorite: function(el, ev) {
		...
		this.rest(SocialAPI.SocialFavoriteService.REMOVE, [socialAlias], function(data) {
			...
			$(el).removeAttr('disabled');
			FLUIGC.toast({
				message: "${i18n.getTranslation('error.removing')}",
				type: 'danger'
			});
		});
	},
	...
	buildGrid: function() {
		...
		cfg.obj = {
			...
			colNames: ["", "", "${i18n.getTranslation('name')}", "${i18n.getTranslation('type')}"], // nomes das
			// colunas do
			// grid
	...
	removeSelected: function() {
		...
		this.rest(SocialAPI.SocialFavoriteService.REMOVELIST, [alias], function(data) {
			FLUIGC.toast({
				message: "${i18n.getTranslation('successfully.removed')}",
				type: 'success'
			});
			self.loadGrid();
