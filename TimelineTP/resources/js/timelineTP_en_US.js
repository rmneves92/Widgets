var SocialTimeline = SuperWidget
	.extend({
		...
		validImagesFilesTypes: ['jpeg.png', 'jpg.png', 'png.png', 'gif.png'],
		validVideosFilesTypes: ['flv.png', 'mp4.png', '.flv', '.mp4'],
		mentionsObj: null,
		...
		showVideos: function() {
			var allVideos = $("div.start-video-player", this.DOM);
			
			for (var i=0; i < allVideos.length; i++) {	
				var video = $(allVideos[i]);	
				
				FLUIGC.player("#" + video.prop("id"), video.data("url") + "?.mp4", {
			        ratio: 9/16
			    });
				video.removeClass("start-video-player");
			}
		},
		...
		openDocumentAction: function(el, ev) {
			var that = this, $el = $(el), documentId = $el.data('timeline-document-id'), version = $el
				.data('timeline-document-version');

			WCMAPI.Read({
				type: "GET",
				url: ECM.restUrl + "documentView/contentVersion/" + documentId + "/" + version,
				async: false,
				success: function(doc) {
					if (doc != undefined && doc.documentType == "1") {
						window.location = WCMAPI.getProtectedContextPath() + "/" + WCMAPI.tenantCode
							+ "/ecmnavigation?app_ecm_navigation_folder=" + documentId;
					} else {
							...
							getDocument: function() {
								setTimeout(function() {
									...
									if (doc != undefined) {
										...
										ECM.documentView.getDocument(documentId, version);
									}
								}, 500);
							},
							callBack: function() {
		...
		serviceListPosts: function(cb) {
			var lastPostId = $('[data-post-id]:last', this.DOM).data('post-id'), options = {
				url: '/api/public/social/post/listSortedPost/' + this.alias,
				contentType: 'application/json',
				dataType: 'json',
				data: {
					lastPostId: lastPostId || 0,
					limit: this.limit || 20,
					orderingType: this.orderType || 'LAST_UPDATE',
				}
			};
