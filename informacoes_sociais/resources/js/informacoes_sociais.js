	...
	.extend({
		...
		getInfoCommunity: function(alias, callback) {
			...
			this.serviceSocialCommunity(alias, function(err, data) {
				if (err) {
					FLUIGC.toast({
						message: '${i18n.getTranslation("message.error.get.permissions.info")}',
						type: 'danger'
					});
					return false;
				}
				...
				success: function(data) {
					...
					if (data === 'ACCEPTED') {
						...
						$(btnReqFollow).attr('disabled', 'disabled').text(
							"${i18n.getTranslation('card.user.request.pending')}");
					}
				},
		...
		errorMessage: function(oObj, XMLHttpRequest, data, errorThrown, info) {
			...
			if (oObj.status && oObj.status == 403) {
				title = "${i18n.getTranslation('action.canceled')}";
				social1 = (data.social1 || 'social 1').bold();
				social2 = (data.social2 || 'social 2').bold();
				msg += social1 + " " + "${i18n.getTranslation('and')}" + " " + social2 + " "
					+ "${i18n.getTranslation('message.error.cloud.follow')}";
			} else {
				title = '';
				msg = err;
			}
		...
		removeParticipationAction: function(el, ev) {
			...
			FLUIGC.message.confirm({
				message: "${i18n.getTranslation('confirm.remove.participation')}",
				title: "${i18n.getTranslation('remove.participation')}",
				labelYes: "${i18n.getTranslation('confirm.yes')}",
				labelNo: "${i18n.getTranslation('confirm.no')}"
									...
									function(data) {
										...
										if (!$('[data-participate-action-popover]', parent).length) {
											$('.container-social-actions', parent)
												.prepend(
													'<button class="totvs-btn-relation mini" data-participate-action-popover>${i18n.getTranslation("participate")}</button>');
										}
									}, that.errorMessage);
						}
					});
		...
		sendMessage: function(el, ev) {
			...
			that.createMessage([$(el).attr('userId')], $(el).attr('userName'), function(err, data) {
				if (err) {
					FLUIGC.toast({
						message: "${i18n.getTranslation('message.error.start.conversation')}",
						type: "danger"
					});					
				} else {
		...
		saveDescription: function(el, ev) {
			...
			if (/<[a-z][\s\S]*>/i.test(description)) {
				...
				WCMC.messageWarn("${i18n.getTranslation('description.specialChar')}");
				return false;
			}
		...
		sendUser: function(users) {
			...
			that.rest(SocialAPI.CommunityService.ADDSOCIAL, [communityId, usersList], function() {
				...
				socialGlobal.message({
					text: '${i18n.getTranslation("community.alert.user.success")}',
					type: 'success',
					timeout: 2000
				});
				...
				function(oObj, XMLHttpRequest, data, errorThrown, info) {
					...
					if (oObj.status == 403 && data && data.origin && data.origin == 'SocialAccessControl') {
						...
						var social1 = (data.social1 || 'social 1').bold(), social2 = (data.social2 || 'social 2')
							.bold(), messageTitle = '${i18n.getTranslation("action.canceled")}', messageError = social1
							+ " " + '${i18n.getTranslation("and")}' + " " + social2 + " "
							+ '${i18n.getTranslation("message.error.cloud.follow")}';
		...
		sendGroup: function(groups) {
			...
			that.rest(SocialAPI.CommunityService.ADDGROUP, [communityId, groupList], function() {
				...
				socialGlobal.message({
					text: '${i18n.getTranslation("community.alert.group.waiting")}',
					type: 'success',
					timeout: 2000
				});
			},
