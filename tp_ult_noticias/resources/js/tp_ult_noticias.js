var Alertas = SuperWidget.extend({
    ...
    serviceGetSettings: function() {
        var _this = this,
            settings = {
                url: "/api/public/ecm/document/getDocumentByDatasetName/alertas",
                type: "GET",
                cache: false
            };
    ...
    serviceGetNews: function() {
        ...
        DatasetFactory.getDataset("alertas", null, constraints, orderByConstraint, callback)
    },
        ...
        if (news != null && news.values.length > 0) {
            ...
            if (docId) {
                ...
                for (i = 0; i < news.values.length; i++) {
                	if(news.values[i].idImageECM != null && news.values[i].idImageECM != ''){
                		news.values[i].imgUrl = WCMAPI.getServerURL() + "/webdesk/streamcontrol/padrao.png?WDCompanyId=" + WCMAPI.getTenantId() + "&WDNrDocto=" + news.values[i].idImageECM + "&WDNrVersao=1000";
                	}
                	else{
                		...
                		if(news.values[i].conteudo_min != null)
                			news.values[i].conteudo_min = (news.values[i].conteudo_min.length > 150) ? news.values[i].conteudo_min.substr(0, 600).concat("...") : news.values[i].conteudo_min;
                	}
    ...
    showNewsDetail: function(el, ev) {
    	...
    	$(".modal-body").closest('div').find('.text-center').append("<div class='div_alertas'>");
    	$(".div_alertas").append(x);
    	//===========================
    },
    ...
    saveSettings: function(el, ev) {
        ...
        WCMSpaceAPI.PageService.UPDATEPREFERENCES({
            ...
            fail: function(xhr, message, errorData) {
                FLUIGC.toast({
                    title: '${i18n.getTranslation("alertas.error.saving.preferences")}',
