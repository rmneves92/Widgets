		...
		.extend({
			...
			updatePreferences: function(el, ev) {
				...
				if (hasError == null) {
					this.rest(WCMSpaceAPI.PageService.UPDATEPREFERENCES, [that.instanceId, args], function(res) {
						if (res) {
							FLUIGC.toast({
								message: "¡Widget actualizado con éxito!",
								type: "success"
							});
						}
						if(el)
					...
					FLUIGC.toast({
						title: "No se pudo actualizar la widget. Póngase en contacto con el administrador.",
						message: hasError,
						type: "warning"
					});
			...
			optionCloudRadio : function(){
				...
				if(radioVal == 'optGeneral') {
					...
					$('#divCommunity').show();
					this.showMessage('solo aparecerán tags de publicaciones de la comunidad seleccionada');
					...
					if(err) {
						for(var i=that.countSearch; i <1; i++){
							...
							that.serviceSearchAdvanced(function(err2, data2) {
								if(err2) {
									that.showMessage('Nube de tags no disponible en este momento.');
									return false;
								}
								that.findTagsCloud();
								return false;
			...
			loadTagsCloud: function(content) {
				...
				if(words.length < this.minSizeCloud) {
					$('#cloud').hide();
					this.showMessage('No hay tags suficientes para mostrar la nube de tags.');
					return;
				}
			...
			validateCommunity: function() {
				if(this.communityRest != null) {
					if(this.communityRest.hidden) {
						$('#cloud').hide();
						$('#edit').hide();
						this.showMessage('Compatible solo con comunidades públicas.');
						return;
					}
						...
						$("#community"+ this.instanceId).autocomplete({
							source: function(request, response) {
								that.serviceListCommunity(request, function(err, data) {
									if(err) {
										FLUIGC.toast({
											message: "Error al buscar las comunidades. Póngase en contacto con el administrador. " + data.message,
											type: 'danger'
										});
										return false;
									}
							...
							select: function(event, ui) {
								if(ui.item.hidden) {
									FLUIGC.toast({
										message: "Compatible solo con comunidades públicas. ",
										type: 'danger'
									});
								} else {
			...
			findCommunity: function() {
				...
				that.serviceFindCommunity(alias, function (err, data) {
					if(err) {
						FLUIGC.toast({
							message: "Error al buscar la comunidad. Póngase en contacto con el administrador. " + data.message,
							type: 'danger'
						});
						return false;
					}
