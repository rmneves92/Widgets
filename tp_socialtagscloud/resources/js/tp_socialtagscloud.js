var socialtagscloud = SuperWidget
		.extend({
			...
			updatePreferences: function(el, ev) {
				var that = this;
				var args = {}; 
				var $community = $('[data-community]',this.DOM); 
				var $lastTagsDay = $('[data-last-tags-day]',this.DOM); 
				that.bordColor = $('#bordColor_' + that.instanceId, this.DOM).val();
				var hasError = null;
				var btn = $(el);
				btn.prop('disabled', true);
				
				console.log("____BordColor____" + that.bordColor);

				this.community = $community.val();
				this.lastTagsDay = $lastTagsDay.val();
				console.log(this.bordColor);


				args['community'] = this.community;
				args['lastTagsDay'] = this.lastTagsDay;
				args['bordColor'] = that.bordColor;
				...
				if (hasError == null) {
					...
					FLUIGC.toast({
						title: "${i18n.getTranslation('msg.error.update')}",
						message: hasError,
						type: "warning"
					});
			...
			optionCloudRadio : function(){
				...
				if(radioVal == 'optGeneral') {
					...
					$('#divCommunity').show();
					this.showMessage('${i18n.getTranslation("msg.info.communiypage")}');
					...
					if(err) {
						for(var i=that.countSearch; i <1; i++){
							...
							that.serviceSearchAdvanced(function(err2, data2) {
								if(err2) {
									that.showMessage('${i18n.getTranslation("msg.error.tagscloud")}');
									return false;
								}
								that.findTagsCloud();
								return false;
			...
			loadTagsCloud: function(content) {
				...
				if(words.length < this.minSizeCloud) {
					$('#cloud').hide();
					this.showMessage('${i18n.getTranslation("msg.enoughtags")}');
					return;
				}
			...
			validateCommunity: function() {
				if(this.communityRest != null) {
					if(this.communityRest.hidden) {
						$('#cloud').hide();
						$('#edit').hide();
						this.showMessage('${i18n.getTranslation("compatibility.community")}');
						return;
					}
						...
						$("#community"+ this.instanceId).autocomplete({
							source: function(request, response) {
								that.serviceListCommunity(request, function(err, data) {
									if(err) {
										FLUIGC.toast({
											message: "${i18n.getTranslation('msg.error.listcommunity')} " + data.message,
											type: 'danger'
										});
										return false;
									}
							...
							select: function(event, ui) {
								if(ui.item.hidden) {
									FLUIGC.toast({
										message: "${i18n.getTranslation('compatibility.community')} ",
										type: 'danger'
									});
								} else {
			...
			findCommunity: function() {
				...
				that.serviceFindCommunity(alias, function (err, data) {
					if(err) {
						FLUIGC.toast({
							message: "${i18n.getTranslation('msg.error.findcommunity')} " + data.message,
							type: 'danger'
						});
						return false;
					}
